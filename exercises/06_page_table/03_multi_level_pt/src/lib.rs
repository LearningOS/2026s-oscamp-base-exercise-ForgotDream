...
impl Sv39PageTable {
    ...
    pub fn map_page(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        for level in (1..=2).rev() {
            ...
            let pte = self.nodes[&curr_ppn].entries[vpn];

            if pte & PTE_V == 0 {
                let new_ppn = self.alloc_node();
                let new_pte = (new_ppn << PPN_SHIFT) | PTE_V;
                self.nodes.get_mut(&curr_ppn).unwrap().entries[vpn] = new_pte;
                curr_ppn = new_ppn;
            } else {
        ...
        let pa_ppn = pa / (PAGE_SIZE as u64);
        let pte = (pa_ppn << PPN_SHIFT) | flags | PTE_V;

        self.nodes.get_mut(&curr_ppn).unwrap().entries[vpn0] = pte;
    }
    ...
    pub fn translate(&self, va: u64) -> TranslateResult {
        ...
        for level in (0..=2).rev() {
            ...
            if pte & (PTE_R | PTE_X | PTE_W) != 0 {
                ...
                let pa = (ppn * PAGE_SIZE as u64) | offset;
    ...
    pub fn map_superpage(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        let pte = self.nodes[&curr_ppn].entries[vpn2];

        if pte & PTE_V == 0 {
            let new_ppn = self.alloc_node();
            let new_pte = (new_ppn << PPN_SHIFT) | PTE_V;
            self.nodes.get_mut(&curr_ppn).unwrap().entries[vpn2] = new_pte; 
            curr_ppn = new_ppn;
        } else {
            curr_ppn = pte >> PPN_SHIFT;
        }
        ...
        self.nodes.get_mut(&curr_ppn).unwrap().entries[vpn1] = pte;
    }
}
