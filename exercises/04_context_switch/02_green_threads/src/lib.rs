...
impl Scheduler {
    ...
    fn schedule_next(&mut self) {
        ...
        let mut new_idx = (old_idx + 1) % self.threads.len();

        while new_idx != old_idx && self.threads[new_idx].state != ThreadState::Ready {
            new_idx = (new_idx + 1) % self.threads.len();
        }
        ...
        if self.threads[new_idx].entry.is_some() {
            unsafe { CURRENT_THREAD_ENTRY = self.threads[new_idx].entry; }
        }

        let old_ptr = self.threads[old_idx].ctx.as_mut_ptr();
        let new_ptr = self.threads[new_idx].ctx.as_ptr();

        unsafe { switch_context(&mut *old_ptr, &*new_ptr); }
    }
}
