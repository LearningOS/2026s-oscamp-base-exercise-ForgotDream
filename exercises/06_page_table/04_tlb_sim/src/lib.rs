...
impl Tlb {
    ...
    pub fn insert(&mut self, vpn: u64, ppn: u64, asid: u16, flags: u64) {
        ...
        let current_entry = &mut self.entries[self.fifo_ptr];
        current_entry.valid = true;
        current_entry.vpn = vpn;
        current_entry.ppn = ppn;
        current_entry.asid = asid;
...
impl Mmu {
    ...
    pub fn translate(&mut self, vpn: u64) -> Option<u64> {
        ...
        match self.tlb.lookup(vpn, self.current_asid) {
            Some(ppn) => return Some(ppn),
            None => {}
        };

        for (asid, pm) in &self.page_table {
            if *asid == self.current_asid && pm.vpn == vpn {
                self.tlb.insert(pm.vpn, pm.ppn, *asid, pm.flags);
                return Some(pm.ppn);
            }
        }
